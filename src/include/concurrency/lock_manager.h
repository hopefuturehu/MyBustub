...
class LockManager {
 public:
  ...
  /**
   * @brief Check the rationality of the table lock
   * @return true for success, if false will throw error
   */
  auto CheckLockValidity(Transaction *txn, LockMode lock_mode) -> bool;

  auto ChecktTableLockUpgrade(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool;
